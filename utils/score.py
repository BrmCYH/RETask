...
def extract_items(tokens_in, id2predicate, model):
    R = []
    # _t=
    ...
    #使用模型的predict_subj_per_instance函数
    _s1, _s2, hidden, sentence_rep = model.predict_subj_per_instance(_t)
...
def evaluate(data, model):
    ...
    for d in tqdm(iter(data)):
        R = extract_items(d['tokens'],d['pos_tags'], model)
