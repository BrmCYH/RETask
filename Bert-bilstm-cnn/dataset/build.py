    ...
    for files in dirx:
        ...
        if files.split('.')[-2]in['train','test','dev']:
            ...
            with open(os.path.join(args['dir'],files),'r',encoding="utf-8")as f1:
                with open(os.path.join(args['dir'],"{}_{}.json".format(filename,files.split('.')[0])),"a+",encoding="utf-8")as f:
                    listx=json.load(f1)
                    for itm in listx:
                        # dictx={"context":" ".join(itm['tokens']),"triple":chuli(itm,tokenizer=tokenizer)}
                        json.dump({"context":" ".join(itm['tokens']),"triple":chuli(itm,opt=opt,tokenizer=tokenizer,)},f,ensure_ascii=False)     
                        f.write("\n")
import argparse
if __name__=="__main__":
    parser=argparse.ArgumentParser()
    parser.add_argument("--dir",type=str,default='Bert-bilstm-cnn/dataset/WebNLG/data')
