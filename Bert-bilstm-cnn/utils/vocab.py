...
class Vocab(object):# GloVe词表操作
    def __init__(self, filename, load=False, word_counter=None, threshold=0):
        if load:  #load ==True
            ...
            self.id2word, self.word2id = self.load(filename) # 词与id
            self.size = len(self.id2word) #词表长度
            print("Vocab size {} loaded from file".format(self.size))
        else: 
            print("Creating vocab from scratch...")
            # 如果word_counter 非None
    ...
    def load(self, filename):
        ...
        with open(filename, 'rb') as infile:
            id2word = pickle.load(infile)# infile文件中的内容为id:word
