...
class SubjTypeModel(nn.Module):
    ...
    def __init__(self, opt, input_size, hidden_dim, posi_emb,filter=3):
        super(SubjTypeModel, self).__init__()
        self.input_size = input_size
        self.dropout = nn.Dropout(opt['dropout'])
        self.hidden_dim = hidden_dim
        # 位置编码
        self.position_embedding = nn.Embedding(posi_emb, opt['position_emb_dim'])
        # 开始的lstm
        self.rnn_start = nn.LSTM(self.input_size, hidden_dim//2, 1, batch_first=True, bidirectional=True)
        # 后来的lstm
        self.rnn_end = nn.LSTM(self.input_size, hidden_dim//2, 1, batch_first=True, bidirectional=True)
        #线性层
...
class ObjBaseModel(nn.Module):
    ...
    def __init__(self, opt, input_size, hidden_dim, posi_emb,filter=3):
        ...
        self.distance_to_subj_embedding = nn.Embedding(posi_emb, opt['position_emb_dim'])
        self.distance_to_obj_start_embedding = nn.Embedding(posi_emb, opt['position_emb_dim'])
